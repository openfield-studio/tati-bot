...
## 検証結果が出た後の多角的検証（2026-09-17〜）

単一の切り口(例: 3分位で全期間プール)の結果だけで結論を出さず、**同じデータから
以下のような別angleの再検証を標準的に行う**（新規データ取得なしでできるものが多い）:

1. **分位の細かさを変える**: 3分位→5分位(クインタイル)にして、より極端な群同士の
   比較でも効果の方向・大きさが変わらないか(new_factors_angles.pyのangle A参照)
2. **既存の有力ファクターと組み合わせる**: 例えば割安さ(PER)と組み合わせて
   「割安な銘柄群の中だけ」で見た時に追加の予測力があるか(同ファイルのangle B参照)
3. **相場局面を絞る**: 通し期間の結果が特定の相場環境(上げ相場/下げ相場)に
   引っ張られていないか、defensive_stock_lab.pyのような実際の急落局面だけに
   絞った再検証も行う(new_factors_crash_lab.py参照)

3分位・全期間プールの結果だけを見て「効いた/効かなかった」と即断せず、上記のような
角度替えで方向性が保たれるか(頑健性)を確認してからSESSION_LOG.mdに記録する。
...
- **追加の質フィルター(2026-09-16)**:
  - 配当性向<=100%(利益より配当が多い=減配リスクを回避)
  - 利益成長率>=-20%(前年比20%超の減益は「安いだけ」の罠として除外)
  - 非金融業は負債比率(D/E)<=200%(過大な借入を回避。銀行・証券・保険は業態上
    D/Eが高いのが通常なので対象外、yfinanceのsector="Financial Services"で判定)
- **反発シグナル(2026-09-16追加)**: 割安なだけで下げ続けている銘柄を避けるため、
  ...
  **RSI70以上(OVERBOUGHT_RSI)は「既に反発しきった」としてランキング対象から完全に除外**
  (パーセンタイル計算のプールにも含めない)。これも将来予測ではなく単純なテクニカルルール。
...
- **(2026-09-16) 過去データでの簡易検証結果**: 業種単位の歴史的割安さ(JPX公表データ、
  13年分)は予測力なし(NO-GO、SESSION_LOG項目26)。一方、**個別銘柄のPER割安さ
  (yfinance決算データから再構築、5年分)は将来リターンとプラスの関係**が見られた
  (外れ値除外後、12ヶ月後で割安グループ+31.3% vs 割高グループ+12.5%、
  SESSION_LOG項目27)。ただし期間が短い・生存バイアスあり・DSR未補正のため、
  まだ「検証済みの優位性」と呼べる段階ではない。
