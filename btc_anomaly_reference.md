...
| ソース | 確認できたこと | 確認できなかったこと | 個人の研究用途での判断 |
|---|---|---|---|
| **Bitstamp** | 公開APIは認証不要、レート制限は「1秒400リクエスト、10分10,000リクエスト」。**商用利用(自社の事業目的)はData License Agreementの締結が必要**と明記。 | 利用規約(Terms of Use)へのリンクはAPIページに無く、個人利用の可否を明記した条項は未確認 | 商用ではない個人の研究・バックテストなら、明記された禁止は見当たらない。**再配布・商用はしない**こと |
| **bitbank** | 利用規約(https://bitbank.cc/doc/tos)は口座利用者向け。第20条で本サービスのコンテンツの知的財産権は当社帰属、第21条の秘密情報は「既に公知の情報」を除外。禁止行為の確認範囲に、公開チャートデータの取得・保存を禁じる条項は無し。API文書にもレート制限・利用規約の記載なし | 禁止行為の全文(一部のみ確認)、口座を持たない利用者への適用範囲 | 個人の分析目的で日足〜1分足を取得することに明記された禁止は見当たらない。**再配布はしない**。取得は1日1リクエストなので間隔を空けて負荷をかけない |
| **Binance** | 公開データリポジトリ(binance/binance-public-data)のライセンスはMIT。検索結果では「市場データを使ったデータ配信サービスや、市場データで収益を得るサービスは禁止」との記述 | Binance本体の利用規約(https://www.binance.com/en/terms)は本文が描画されず読めなかった。**データそのものの規約は未確認** | 商用・再配布をしない個人利用なら問題になりにくいが、**規約本文を読めていないので未確認扱い**。第一候補にはしない |

**方針**: 生データはgit管理せず(.gitignore)、再配布しない。結果(統計量)だけを記録する(J-Quantsと同じ運用)。
第一候補はBitstamp(1分足2012年〜、規約上の明示的な個人禁止なし)、円建てで見るならbitbank(2018年〜)。
Binanceは規約本文を確認できるまで使わない。

### Quantpedia裏取り(2026-09-22): 元論文の数字と、自前検証の食い違い

Quantpedia無料枠で「Overnight Seasonality in Bitcoin」(#0753)を確認。元論文は
Padyšák & Vojtko (2022) "Seasonality, Trend-following, and Mean reversion in Bitcoin"
(SSRN 4081000)。戦略定義は**UTC22:00に買い、2時間保有(22時+23時、24時に手仕舞い)**。
バックテスト期間2015〜2021年、**年率+33%・Sharpe1.58・最大DD-34%**(Gemini取引所データ、
コスト控除の有無は不明)。Quantpediaは「Confidence in Anomaly's Validity: Strong」評価。

**自前検証(項目72、Bitstamp・2015〜2026・コスト控除後)との比較**:
- 同じ「22+23時・2時間保有」の組み合わせで、こちらはグロス+3.4bps/日(年率換算で
  1年33%どころか年率十数%にも届かない水準)、コスト控除後は全年マイナス。
- 元論文の期間(〜2021)より新しいデータ(2022〜2026)を足すと効果が大きく縮小している
  可能性が高い。アノマリーは論文公表後に効果が薄れる(McLean & Pontiff 2016の
  publication decay)という一般的なパターンと整合的。
- 取引所が違う(Gemini vs Bitstamp)ことも数値差の一因かもしれないが、規模の違い
  (年率33% vs 一桁台)は取引所差だけでは説明しにくい。

**結論は変わらない**: 「22時台に何らかの偏りがある」こと自体は自前検証でも支持されたが、
「コスト後に儲かる」という元論文の主張は、より長い期間・別取引所のデータでは再現しなかった。
公開されている「強気の結論」を鵜呑みにせず自分で検証し直す価値があった実例。
