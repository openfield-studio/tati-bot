...
# -*- coding: utf-8 -*-
"""
power_pro_lab.py — 1627(電力・ガスETF)版 プロの検証4点セット（記録用・本体には組み込まない）
==================================================================
1306のpro_lab.pyで行った4点セット(パラメータ感度・モンテカルロ・運テスト・
コスト感度)を、power_etf_lab.pyでGO判定された1627戦略(買い<35/利確>50/
損切り5%)にもかけて、まぐれでないか頑健性を確認する。

使い方: python power_pro_lab.py
"""
import random
import statistics
...
prices = [float(x) for x in yf.Ticker("1627.T").history(period="10y")["Close"].dropna().tolist()]
...
print(f"データ: 1627(電力・ガスETF) {len(prices)}本(10年)")
...
print(f"\n④ コスト感度")
for mult in (1, 2, 3, 5):
    c2 = ResearchConfig(symbol="1627", commission_bps=cfg.commission_bps * mult,
